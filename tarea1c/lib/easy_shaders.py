...
def textureChessboardSimpleSetup(texture, n):
    ...
    image[:, 0] = np.array([87,138,52, 255], dtype=np.uint8)
    image[:, -1] = np.array([87,138,52, 255], dtype=np.uint8)
    image[0, :] = np.array([87,138,52, 255], dtype=np.uint8)
    image[-1, :] = np.array([87,138,52, 255], dtype=np.uint8)
    for i in range(1, n-1):
        for j in range(1, n-1):
            if (i + j) % 2 == 0:
                image[i][j] = np.array([162, 209, 73, 255], dtype=np.uint8)
            else:
                image[i][j] = np.array([170, 215, 81, 255], dtype=np.uint8)

    internalFormat = GL_RGBA
    format = GL_RGBA

    glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, image.shape[0], image.shape[1], 0, format, GL_UNSIGNED_BYTE, image)

def chessboard2GPUShape(shape, n, wrapMode=None, filterMode=None):
    ...
    textureChessboardSimpleSetup(gpuShape.texture, n)
